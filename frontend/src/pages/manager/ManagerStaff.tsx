...
import { Users, Plus, Search, Pencil, ShieldOff, ShieldCheck, X, Eye, EyeOff, Trash2, Lock, KeyRound, AlertTriangle } from 'lucide-react';
...
interface User {
  ...
  hasPin?: boolean;
  /** Whether a website password is on file (cashiers sign in with one). */
  hasPassword?: boolean;
}
 ...
 *
 * The card still offers a PIN to every role it can edit, so a cashier can
 * unlock a handheld with a 4-digit code too; only the app-first roles need one
 * to sign in.
 */
const PIN_CAPABLE_ROLES = ['WAITER', 'COOKER', 'BARISTA', 'CASHIER'];
const PIN_SIGNIN_ROLES = ['WAITER', 'COOKER', 'BARISTA'];
...
/**
 * Roles a manager is allowed to create, edit, or deactivate. Managers cannot
 * manage other managers (or owners) — the server never returns them here.
 */
...
export const ManagerStaff: React.FC = () => {
  ...
  useEffect(() => {
    setPageTitle({ title: t('manager.pageTitle'), subtitle: t('manager.pageSubtitle') });
    setShowDateRange(false);
    return () => {
      setPageTitle({ title: t('manager.revertTitle'), subtitle: '' });
      setShowDateRange(false);
    };
  ...
  /** A manager may only touch staff below them — never owners or other managers. */
  const canManage = (u: User) => (MANAGEABLE_ROLES as readonly string[]).includes(u.role);

  const users: User[] = useMemo(() => {
    const list = ((serverUsers as User[]) || []).filter((u) => u.role !== 'OWNER');
    return list.map((u) =>
  ...
  // Managers only receive the staff below them — the owner (and other
  // managers) never appear — so the manager's own account is counted back in.
  const totalStaff = users.length + (currentUser ? 1 : 0);
  ...
  // The field is offered to every editable role; only app-first roles are
  ...
  const handleSave = async () => {
    ...
    if (!(MANAGEABLE_ROLES as readonly string[]).includes(form.role)) {
      addToast({ type: 'error', title: t('errors.notAllowed'), message: t('errors.notAllowedMsg') });
      return;
    }
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          {manageable ? (
                            ...
                            <Tooltip label={t('row.managerLocked')}>
                              <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[11px] font-medium text-muted-foreground bg-secondary/50">
